...
export const toggleChecked = (id, projectName) => {
  const index = getTodoIndex(projects[projectName], id);
  const { isChecked } = projects[projectName][index];

  projects[projectName][index].isChecked = !(isChecked);
  updateStorage();
};

// private helpers

const getTodoIndex = (todoList, id) => todoList.findIndex((todo) => todo.id === id);
...
const parseFormData = (formData) => {
  ...
  const id = Number((formData.get('id')) ? formData.get('id') : nextValidID += 1);
